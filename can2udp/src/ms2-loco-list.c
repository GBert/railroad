...
static unsigned char M_SIMU_GUI[] = { 0, 0, 0, 0, 3, 8, 0xFF, 0xFF };
static unsigned char M_SIMU_MAS[] = { 0x4D, 0x53, 0x32, 0x01 };		// MS serno 1
...
int get_loco_list(struct ms2_data_t *ms2_data) {
	...
	show_loco_names(input_fd, ms2_data->loco_list_low, ms2_data->loco_list_len);
    fclose(input_fd);
...
int main(int argc, char **argv) {
	...
	if (FD_ISSET(ms2_data.sc, &read_fds)) {
		...
		case 0x40:
		    if (frame.can_dlc != 8)
			continue;
		    if ((frame.can_id & 0xFFFF) != ms2_data.hash)
			waitstate = W_IDLE;
