// ddl_nmra.c - adapted for basrcpd project 2018 - 2023 by Rainer Müller
                        ...
                        int offset)
{
    ...
    if (j > 0) {
    	send_packet(busnumber, packetstream, QNBACCPKT, 2);
        return 0;
    }
...
void comp_nmra_multi_func(bus_t busnumber, gl_data_t *glp)
{
    ...
    if (j > 0 && jj > 0) {
        update_NMRAPacketPool(busnumber, glp, packetstream, j,
                              packetstream2, jj);
    	send_packet(busnumber, packetstream, QNBLOCOPKT, 2);
        if (nfuncs && (nspeed > 14)) {
    		send_packet(busnumber, packetstream, QNBLOCOPKT, 2);
        }
    }
                                      ...
                                      int value, int mode)
{
    ...
    if (j > 0) {
    	send_packet(busnumber, packetstream, QNBLOCOPKT, 3);
        return value;
    }
    return -1;
}
...
static char idlepacket[] = {NORM_PREA, 3, 0xFF, 0x00, 0xFF};
static char resetpacket[] = {LONG_PREA, 3, 0x00, 0x00, 0x00};
...
{
	send_packet(busnumber, idlepacket, QNBLOCOPKT, nmbr);
}
...
/**
  ...
  @par Input: bus_t busnumber
              ...
              int value
              int verify
  @return 1 if acknowledged, 0 otherwise, < 0 indicates error
*/
                                          ...
                                          int value, int verify)
{
    ...
    /* no special error handling, it's job of the clients */
    if (cv < 0 || cv > 1023 || value < 0 || value > 255) return -1;

    /* address only mode */
//	if (!cv && (value < 128)) {		HACK: disabled because not yet adapted !
//		return protocol_nmra_sm_phregister(busnumber, 1, value, verify);
//	}

	if (send_packet(busnumber, resetpacket, QNPROGPKT, 10) > 5)
		return -2;					// ack erroneously active
	progpacket[0] = LONG_PREA;
	progpacket[1] = 4;
	progpacket[3] = cv;
	progpacket[4] = value;
	if (!verify) {					// SET
		progpacket[2] = 0x7c | (cv >> 8);
		progpacket[5] = progpacket[2] ^ progpacket[3] ^ progpacket[4];
		acks = send_packet(busnumber, progpacket, QNPROGPKT, 10);
	}
	else {							// VERIFY
		progpacket[2] = 0x74 | (cv >> 8);
		progpacket[5] = progpacket[2] ^ progpacket[3] ^ progpacket[4];
		acks = send_packet(busnumber, progpacket, QNPROGPKT, 6);
	}
	send_packet(busnumber, resetpacket, QNBLOCOPKT, 2);
	return ((acks > 1) ? 1 : 0);
}
...
/**
  ...
  @return 1 if acknowledged, 0 otherwise, < 0 indicates error
*/
static int protocol_nmra_sm_direct_cvbit(bus_t busnumber, int cv, int bit,
                                         int value, int verify)
{
    /* direct cv access */
	char progpacket[8];
	...
	progpacket[0] = LONG_PREA;
	progpacket[1] = 4;
	progpacket[2] = 0x78 | (cv >> 8);
	progpacket[3] = cv;
	if (!verify) {					// SET
		progpacket[4] = 0xf0 | (value << 3) | bit;
		progpacket[5] = progpacket[2] ^ progpacket[3] ^ progpacket[4];
		acks = send_packet(busnumber, progpacket, QNPROGPKT, 10);
	}
	else {							// VERIFY
		progpacket[4] = 0xe0 | (value << 3) | bit;
		progpacket[5] = progpacket[2] ^ progpacket[3] ^ progpacket[4];
		acks = send_packet(busnumber, progpacket, QNPROGPKT, 6);
	}
	send_packet(busnumber, resetpacket, QNBLOCOPKT, 2);
